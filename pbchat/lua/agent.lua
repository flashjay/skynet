...
local addr = io.open("pbchat/chat.pb","rb")
...
skynet.register_protocol {
    ...
    dispatch = function (_, _, pack)
        ...
        local msg = protobuf.decode("chat", pack)
        
        print(string.format(" id = %d", msg.id))
        print(string.format(" name = %s", msg.name))
        print(string.format(" text = %s", msg.text))
        
        local retmsg = protobuf.encode("chat", msg)
        
        socket.write(client_fd, netpack.pack(retmsg))
